...
class fssd(Daemon):
	def __init__(self, pidfile, xmlconfig, loglevel=logging.NOTSET):
		...
		if self.config['root']['fssd:config']['fssd:alerts']['fssd:custom-script']
			self.log.error("custom script")
			self.config['fssd:custom-script'] = self.config['root']['fssd:config']['fssd:alerts']['fssd:custom-script']
		else:
			self.config['fssd:custom-script'] = 'False'
	...
	def custom_script(self):
		try:
			if not self.config['fssd:custom-script']:
				self.log.debug("Custom script is disabled")
				return True

			call(self.config['fssd:custom-script']['fssd:script'])
		except socket.error as e:
			self.log.error(format("Exception in custom_script: %d, %s" % (e.errno, e.strerror)))
		except:
			self.log.error("Unexpected error in custom_script:", sys.exc_info()[0])
